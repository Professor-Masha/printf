...
/************** WRITE HANDLE *****************/
/**
 * handle_write_char - A fuction that Prints a string.
 * @c: char types.
...
{ /* char is storedi left and at buffer's right*/
	int i = 0;
	char padd = ' ';
 ...
 * @ind: char types.
 * @buffer: Buffer of array to handle.
...
/**
 * write_num - A fuction that writes using a buffer.
	...
	int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		...
		else if (!(flags & F_MINUS) && padd == '0')/* extra char to left */
		{
			if (extra_c)
...
/**
 * write_unsgnd - it prints unsigned number.
 * @is_negative: IT indicates if num is negative
...
int write_unsgnd(int is_negative, int ind,
	...
	int flags, int width, int precision, int size)
{
	/* The number stored at the bufer's right and at position i */
	...
	if (width > length)
	{
		...
		if (flags & F_MINUS) /* Asign extra char*/
		{
			return (write(1, &buffer[ind], length) + write(1, &buffer[0], i));
		}
		else /* Asign extra char to left */
		{
...
/**
 * write_pointer - A fuction that writes memory address.
